...
class PublicationController extends Controller
{
	...
	//Portal Payments
	public function publicationsmanage($id)
	{
		if(!Auth::check()){
			return redirect()->action([PortalController::class, 'login']);
	    }
	    else{
           $rfps = publications::where("type",$id)->orderby('id',"desc")->get();
	    	return view('portal.publications')->with('rfps',$rfps)->with('id',$id);
	    }
		
	}
	public function addpublications($id)
	{
		if(!Auth::check()){
			return redirect()->action([PortalController::class, 'login']);
	    }
	    else{
	    		return view('portal.addpublications')->with('id',$id);
	    }
	...
	public function editpublications($id)
	{
		if(!Auth::check()){
	    	...
	    	$rfps = publications::where('id',$id)->orderby('id',"desc")->first();
	    		return view('portal.editpublications')->with("rfps",$rfps);
	    }
	...
	public function deletepublications($id)
	{
		if(!Auth::check()){
	    	...
	    	$rfps = publications::where('id',$id)->orderby('id',"desc")->delete();
	...
	public function postpublications(){
		...
		$destinationPath = 'projectsfiles/publications';
		...
		if($jobs->save())
		{
			...
			return Redirect::back()
            ->with('message','You   Successfully Posted a publications!');
		}
	...
	public function updatepublications(){
        ...
        $jobs =  publications::find($data['id']);
		$jobs->title = $data['title'];
		$jobs->country = $data['country'];
		$jobs->city = $data['city'] ?? '';
		$jobs->start_date = $data['start_date'];
